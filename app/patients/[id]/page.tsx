...
function PatientDetailPageContent() {
  ...
  return (
    <div className="w-full max-w-full overflow-x-hidden space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto flex-wrap">
          ...
          <button
            ...
            className="flex-1 sm:flex-initial flex items-center justify-center gap-1.5 px-3.5 py-2.5 sm:py-2 rounded-xl bg-teal-600 hover:bg-teal-700 text-white text-xs font-bold transition-all shadow-xs min-h-[40px] cursor-pointer"
          >
      ...
      <div className="p-4 sm:p-6 rounded-2xl sm:rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xs space-y-4">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="flex items-start gap-3 sm:gap-4">
            <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-gradient-to-tr from-teal-600 to-cyan-500 text-white flex items-center justify-center font-extrabold text-lg sm:text-xl shadow-md shrink-0">
            ...
            <div className="space-y-1 min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-xs px-2.5 py-0.5 rounded-full font-mono bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 shrink-0">
                  ID: #{patient.id.slice(0, 8)}
                </span>
              </div>

              <div className="flex flex-wrap items-center gap-2.5 sm:gap-3 text-xs text-slate-500 dark:text-slate-400">
                {patient.phone && (
                ...
                {patient.address && (
                  <span className="flex items-center gap-1 truncate max-w-[220px] sm:max-w-xs">
          ...
          <div className="grid grid-cols-3 gap-1.5 sm:gap-3 self-stretch sm:self-start lg:self-center p-2.5 sm:p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 text-center sm:text-left">
            <div className="px-1.5 sm:px-3 border-r border-slate-200 dark:border-slate-700">
              <span className="text-[10px] uppercase font-bold text-slate-400 block truncate">Total Billed</span>
              <span className="text-xs sm:text-sm font-mono font-bold text-slate-800 dark:text-slate-200">
                ₱{totalBilled.toLocaleString()}
              </span>
            </div>
            <div className="px-1.5 sm:px-3 border-r border-slate-200 dark:border-slate-700">
              <span className="text-[10px] uppercase font-bold text-emerald-600 block truncate">Total Paid</span>
              <span className="text-xs sm:text-sm font-mono font-bold text-emerald-600">
                ₱{totalPaid.toLocaleString()}
              </span>
            </div>
            <div className="px-1.5 sm:px-3">
              <span className="text-[10px] uppercase font-bold text-rose-600 block truncate">Balance Due</span>
              <span className="text-xs sm:text-sm font-mono font-extrabold text-rose-600">
      ...
      {/* NAVIGATION TABS (Edge-to-edge scrollable on mobile, zero overflow) */}
      <div className="border-b border-slate-200 dark:border-slate-800 overflow-x-auto scrollbar-hide -mx-3.5 sm:mx-0 px-3.5 sm:px-0">
        <nav className="flex space-x-2 sm:space-x-6 min-w-max pb-0.5">
          <button
            onClick={() => setActiveTab("chart")}
            className={`py-3 px-2.5 sm:px-1 text-xs sm:text-sm font-semibold border-b-2 whitespace-nowrap transition-all flex items-center gap-1.5 sm:gap-2 cursor-pointer ${
              activeTab === "chart"
